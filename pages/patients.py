...
def get_layout():
    ...
    return dbc.Container([
        # Stores pour les données d'export (remplacent app.server)
        dcc.Store(id='patients-stats-store'),
        dcc.Store(id='patients-missing-store'),
...
def register_callbacks(app):
    ...
    @app.callback(
        ...
        Output('patients-stats-store', 'data')],
    ...
    def update_datatable(data, current_page, selected_years, selected_age_groups, malignancy_filter):
        ...
        if missing_cols:
            return dbc.Alert(f'Missing columns: {", ".join(missing_cols)}', color='warning'), dash.no_update, None
        
        # Calculer les statistiques par année
        stats_by_year = []
        
        # Filtrer selon les années sélectionnées (déjà filtré ci-dessus)
        years_to_process = sorted(df['Year'].unique()) if len(df) > 0 else []
        ...
        if not stats_by_year:
            return dbc.Alert('No data available for the selected years', color='info'), dash.no_update, None
    ...
    @app.callback(
        ...
        State('patients-stats-store', 'data'),
        prevent_initial_call=True
    )
    def export_csv(n_clicks, selected_years, stats_data):
    ...
    @callback(
        [Output('patients-missing-detail-table', 'children'),
         ...
         Output('patients-missing-store', 'data')],
        [Input('data-store', 'data'), 
         Input('current-page', 'data'),
         Input('year-filter-checklist', 'value'),
         Input('patients-age-filter', 'value'),
    ...
    def patients_missing_detail_callback(data, current_page, selected_years, selected_age_groups, malignancy_filter):
        ...
        try:
            ...
            if not existing_columns:
                return dbc.Alert("No Patients variable found", color='warning'), True, None
    ...
    @callback(
        ...
        State('patients-missing-store', 'data'),
        prevent_initial_call=True
    )
    def export_missing_patients_excel(n_clicks, missing_data):
